...
QString findOnnxFile(const QString& extractDir);
std::vector<cv::Scalar> generateRandomColors(int class_names_num, int numChannels);
std::map<int, std::string> loadClassNamesFromCategories(const QString& categoryDir);
bool load_model(std::map<int, std::string>* classNamesMap,const QString& zipFileDir,const QString& extractTozipFileDir,const QString &folderImagesPath);
#endif //LOADMODEL
