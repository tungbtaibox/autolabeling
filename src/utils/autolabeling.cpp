...
bool AutoLabeling::processSingleImage(
    ...
    try {
        ...
        // Thêm thông tin tổng quan lên ảnh
        std::string info_text = "Detected: " + std::to_string(results.size()) + " objects";
        cv::putText(img, info_text, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 255, 0), 2);
...
int AutoLabeling::autoLabelImages(
    ...
    // Tạo ảnh summary tổng quan
    createSummaryImage(output_path.string(), class_counts, names);
...
void AutoLabeling::createSummaryImage(const std::string& output_dir, 
                                     const std::map<int, int>& class_counts,
                                     const std::unordered_map<int, std::string>& names) {
    try {
        // Tạo ảnh summary với kích thước 800x600
        cv::Mat summary_img = cv::Mat::zeros(600, 800, CV_8UC3);
        
        // Vẽ background
        summary_img.setTo(cv::Scalar(50, 50, 50));
        
        // Vẽ tiêu đề
        std::string title = "Auto Labeling Summary";
        cv::putText(summary_img, title, cv::Point(50, 50), cv::FONT_HERSHEY_SIMPLEX, 1.5, cv::Scalar(255, 255, 255), 2);
        ...
        std::string total_info = "Total Objects Detected: " + std::to_string(total_objects);
        cv::putText(summary_img, total_info, cv::Point(50, 100), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 255, 0), 2);
        
        // Vẽ thông tin từng class
        int y_offset = 150;
        int class_count = 0;
        ...
        for (const auto& [class_id, count] : class_counts) {
            ...
            // Tạo màu cho từng class
            cv::Scalar color = generateRandomColor(3);
            
            // Vẽ thông tin class
            std::string class_info = class_name + " (ID: " + std::to_string(class_id) + "): " + std::to_string(count);
            cv::putText(summary_img, class_info, cv::Point(50, y_offset), cv::FONT_HERSHEY_SIMPLEX, 0.8, color, 2);
            
            // Vẽ bar chart đơn giản
            int bar_width = static_cast<int>((count * 300.0) / std::max(1, total_objects));
            cv::rectangle(summary_img, cv::Point(400, y_offset - 20), cv::Point(400 + bar_width, y_offset + 5), color, -1);
            
            y_offset += 40;
            class_count++;
            
            // Giới hạn số class hiển thị để tránh tràn ảnh
            if (class_count >= 10) {
                break;
            }
        }
        ...
        // Lưu ảnh summary
        std::string summary_path = (fs::path(output_dir) / "summary.jpg").string();
        cv::imwrite(summary_path, summary_img);
        
        std::cout << "  - Summary: " << summary_path << std::endl;
        
    } catch (const std::exception& e) {
        std::cerr << "Error creating summary image: " << e.what() << std::endl;
    }
}